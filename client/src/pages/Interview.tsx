  ...
  useEffect(() => {
    ...
    if (currentStage === 3 && voiceInterview.currentQuestionNumber > 1 && !currentQuestion && !isGeneratingRef.current) {
      ...
      if (questionIndex >= sortedQuestions.length) {
        ...
        // Generate the question and handle the result
        isGeneratingRef.current = true;
        setVoiceInterview(prev => ({ ...prev, isGenerating: true }));
        
        generateVoiceQuestion(voiceInterview.currentQuestionNumber, qaHistory)
          .then((generatedQuestion) => {
            if (generatedQuestion) {
              console.log('✅ Effect: Question generated successfully');
            } else {
              console.error('❌ Effect: Question generation failed');
          ...
          .catch((error) => {
            console.error('❌ Effect: Question generation error:', error);
      ...
      generateVoiceQuestion(1, [])
        .then((generatedQuestion) => {
          if (generatedQuestion) {
            ...
            console.error('❌ First question generation failed');
            // Reset flags on failure
            isGeneratingRef.current = false;
            setVoiceInterview(prev => ({ ...prev, isGenerating: false }));
          }
...
const generateVoiceQuestion = useCallback(async (
  questionNumber: number, 
  qaHistory: Array<{question: string, answer: string}>
) => {
  if (!typedInterview?.id) {
    console.log('⚠️ No interview ID');
    return null;
  }
  ...
  console.log('🎤 START: Generating question', questionNumber);
  ...
  try {
    ...
    const requestBody = {
      ...
      questionNumber,
      isWelcomeQuestion: questionNumber === 1
    };

    console.log('📤 REQUEST:', requestBody);

    const res = await fetch(
      `/api/interviews/${typedInterview.id}/voice-question`, 
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
    ...
    if (!res.ok && questionNumber === 1 && res.status === 409) {
      console.log('⚠️ Q1 exists, refetching');
      if (typeof refetchQuestions === 'function') {
        await refetchQuestions();
      }
      setVoiceInterview(prev => ({ ...prev, currentQuestionNumber: 1 }));
      // Still need to reset the generating flags even in this case
      isGeneratingRef.current = false;
      setVoiceInterview(prev => ({ ...prev, isGenerating: false }));
      return null;
    }
    ...
    console.log('✅ RECEIVED from API:', {
      id: newQuestion.id,
      questionNumber: newQuestion.questionNumber || questionNumber,
      question: newQuestion.question?.substring(0, 100)
    });

    const formattedQuestion = {
      ...newQuestion,
      questionNumber: newQuestion.questionNumber || questionNumber, // Ensure this exists
      stage: 3,
      type: 'voice' as const,
    ...
    setAllQuestions(prev => {
      const withoutDuplicate = prev.filter(q => q.id !== formattedQuestion.id);
      const updated = [...withoutDuplicate, formattedQuestion];
      
      console.log('📝 Updated allQuestions:', {
        previousCount: prev.length,
        newCount: updated.length,
        voiceQuestionsCount: updated.filter(q => q.stage === 3).length
      });
      
      return updated;
    });

    // ⚠️ CRITICAL: Wait for refetch to complete
    ...
    console.log('✅ COMPLETE: Question generation finished');
    return formattedQuestion;

  } catch (error) {
    console.error('❌ ERROR in generateVoiceQuestion:', error);
    toast({
      title: "Error",
  ...
  // MAIN VOICE INTERVIEW LOGIC - Generate first question when entering stage 3
  // --- ⚙️ FIX FOR INITIAL GENERATION useEffect (replaces the one you had) ---
useEffect(() => {
    // Only run in stage 3
    if (currentStage !== 3) {
      return;
    ...
    // Check if we need to generate the first question (Q1)
    ...
    // Check if the welcome question already exists (simplified check)
    const voiceQuestions = Array.isArray(allQuestions) 
      ? allQuestions.filter((q) => Number(q.stage) === 3)
      : [];
    const welcomeQuestionExists = voiceQuestions.length > 0;
    
    // 1. If Q1 exists, just mark the attempt so we don't try to generate again.
    if (welcomeQuestionExists) {
        hasAttemptedFirstQuestionGeneration.current = true;
        // Optionally update hasWelcomeQuestion flag if you use it for UI logic
        setVoiceInterview(prev => ({ ...prev, hasWelcomeQuestion: true })); 
        return;
    }

    // 2. If we need to generate Q1
    if (shouldGenerateFirstQuestion) {
        console.log('🎤 Initializing and generating FIRST voice question');
        ...
        // Generate the question (generateVoiceQuestion handles its own finally block)
        generateVoiceQuestion(1, []); 
    }
}, [currentStage, voiceInterview.currentQuestionNumber, voiceInterview.isGenerating, generateVoiceQuestion, allQuestions]);
  ...
  }, [currentStage, currentQuestion]); // Simplified dependencies

const handleVoiceSubmit = useCallback(async (blob?: Blob) => {
    // 1. Pre-Check and Guards 🚫
    if (isSubmittingVoiceRef.current || isGeneratingRef.current) {
        console.log('⚠️ Voice submit/generate already in progress.');
        return;
    }
    
    if (!currentQuestion || !typedInterview?.id) {
        toast({ 
            title: "Error", 
            description: "No current question or interview found. Please refresh.", 
            variant: "destructive" 
        });
        return;
    }
    ...
    if (!answerText.trim() || answerText.trim().length < 5) {
        toast({ 
            ...
            description: "Please provide a sufficient spoken/typed answer before submitting.", 
            variant: "destructive" 
        });
        return;
    }

    // 2. Set Submission State & Clear TTS 🛑
    ...
    const currentQText = currentQuestion.question;
    ...
    try {
        ...
        console.log(`🔄 Submitting response for Q${voiceInterview.currentQuestionNumber}: ${currentQId}`);
        ...
        // 4. Update QA History
        const newQAPair = { question: currentQText, answer: answerText };
        const updatedQaHistory = [...voiceInterview.qaHistory, newQAPair];
        
        const nextQuestionNumber = voiceInterview.currentQuestionNumber + 1;
        const isLastQuestion = nextQuestionNumber > maxVoiceQuestions;

        // 5. ✅ UPDATE HISTORY ONLY (not question number yet)
        setVoiceInterview(prev => ({ 
            ...prev, 
            qaHistory: updatedQaHistory 
            // Don't update currentQuestionNumber here!
        }));

        if (isLastQuestion) {
            console.log('🏁 Max voice questions reached. Completing interview.');
            await completeInterview();
        } else {
            // 5a. Generate Next Question 🧠
            console.log(`⏭️ Generating NEXT question: ${nextQuestionNumber}`);
            
            isGeneratingRef.current = true;
            setVoiceInterview(prev => ({ ...prev, isGenerating: true }));

            // Generate question and wait for result
            const generatedQuestion = await generateVoiceQuestion(nextQuestionNumber, updatedQaHistory);
            
            // ✅ CRITICAL FIX: Only update number if generation succeeded
            if (generatedQuestion) {
                console.log('✅ Question generated, updating number to', nextQuestionNumber);
                setVoiceInterview(prev => ({ 
                    ...prev, 
        ...
        console.error('❌ Failed during voice submission/generation chain:', error);
        toast({ 
            title: "Error", 
            description: "Failed to submit answer or transition. Please retry.", 
            variant: "destructive" 
        });
        isGeneratingRef.current = false;
        setVoiceInterview(prev => ({ ...prev, isGenerating: false }));
    ...
    currentQuestion, typedInterview?.id, submitResponseMutation, completeInterview, maxVoiceQuestions,
    textAnswer, audioBlob, cameraAnalysisResults, voiceInterview.currentQuestionNumber, 
    voiceInterview.qaHistory, toast, generateVoiceQuestion
]);
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    if (voiceInterview.currentQuestionNumber < maxVoiceQuestions) {
                      ...
                      // Update the voice interview state (only QA history, not question number yet)
                      setVoiceInterview(prev => ({
                        ...prev,
                        qaHistory: updatedQaHistory
                      }));
                      
                      // Generate the next question using the proper function
                      const nextQuestionNumber = voiceInterview.currentQuestionNumber + 1;
                      if (nextQuestionNumber <= maxVoiceQuestions) {
                        // Generate question and wait for result
                        const generatedQuestion = await generateVoiceQuestion(nextQuestionNumber, updatedQaHistory);
                        
                        // Only update question number if generation succeeded
                        if (generatedQuestion) {
                          console.log('✅ Camera results: Question generated, updating number to', nextQuestionNumber);
                          setVoiceInterview(prev => ({ 
                            ...prev, 
                            currentQuestionNumber: nextQuestionNumber
                          }));
                        } else {
                          console.error('❌ Camera results: Question generation returned null');
                          // Reset the generating flags
                          isGeneratingRef.current = false;
                          setVoiceInterview(prev => ({ ...prev, isGenerating: false }));
                          toast({
                            title: "Error",
                      ...
                      // Reset the generating flag before completing
                      isGeneratingRef.current = false;
                      setVoiceInterview(prev => ({ ...prev, isGenerating: false }));
                      await completeInterview();
                    }
